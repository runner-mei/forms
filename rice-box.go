...
func init() {
	...
	file2 := &embedded.EmbeddedFile{
		...
		FileModTime: time.Unix(1489648283, 0),
	...
	filel := &embedded.EmbeddedFile{
		...
		FileModTime: time.Unix(1489648283, 0),
		Content:     string("{{ define \"main\"}}\n<div class=\"form-group{{if .errors}} has-error{{end}}\">\n  {{ if .label }}\n  <label class=\"col-lg-{{default .labelWidth 2}} control-label {{ if .labelClasses }}{{range .labelClasses}} {{.}}{{end}}{{end}}\" {{if\n         .id}} for=\"{{.id}}\" {{end}}>{{.label}}</label>\n  {{end}}\n  <div class=\"col-lg-{{default .controlWidth 9}}\">\n    <input type=\"password\" name=\"{{.name}}\" class=\"form-control{{ if .classes }} {{range .classes}}{{.}} {{end}}{{end}}\"\n           {{if .id}} id=\"{{.id}}\" {{end}}{{if .params}}{{range $k, $v :=.params}} {{$k}}=\"{{$v}}\" {{end}}{{end}}{{if\n           .css}} style=\"{{range $k, $v := .css}}{{$k}}: {{$v}}; {{end}}\" {{end}}{{range $v :=.tags}} {{$v}}{{end}}{{ if\n           .value}} value=\"{{.value}}\" {{end}}>\n    {{if or .helptext .errors }}\n    <span class=\"help-block\">\n      {{if .helptext}}\n        {{ .helptext }}\n      {{end}}\n      {{if .errors}}\n        <ul>\n          {{ range .errors }}<li>{{.}}</li>{{end}}\n        </ul>\n        {{end}}\n    </span>\n    {{end}}\n  </div>\n</div>\n{{end}}"),
	}
	filem := &embedded.EmbeddedFile{
		Filename:    `bootstrap3/text/textareainput.html`,
		FileModTime: time.Unix(1489648283, 0),
		Content:     string("{{ define \"main\"}}\n<div class=\"form-group{{if .errors}} has-error{{end}}\">\n  {{ if .label }}\n    <label class=\"col-lg-{{default .labelWidth 2}} control-label {{ if .labelClasses }}{{range .labelClasses}} {{.}}{{end}}{{end}}\"\n  {{if .id}} for=\"{{.id}}\" {{end}}>{{.label}}</label>\n  {{end}}\n  <div class=\"col-lg-{{default .controlWidth 9}}\">\n  <textarea name=\"{{.name}}\" class=\"form-control{{ if .classes }} {{range .classes}}{{.}} {{end}}{{end}}\" {{if .id}}\n            id=\"{{.id}}\" {{end}}{{if .params}}{{range $k, $v :=.params}} {{$k}}=\"{{$v}}\" {{end}}{{end}}{{if .css}}\n            style=\"{{range $k, $v := .css}}{{$k}}: {{$v}}; {{end}}\" {{end}}{{range $v :=.tags}} {{$v}}{{end}}>{{.text}}</textarea>\n  {{if or .helptext .errors }}\n  <span class=\"help-block\">{{if .helptext}}{{ .helptext }}{{end}}\n    {{if .errors}}<ul>{{ range .errors }}<li>{{.}}</li>{{end}}</ul>{{end}}\n  </span>\n  {{end}}\n  </div>\n</div>\n{{end}}"),
	}
	...
	dir1 := &embedded.EmbeddedDir{
		Filename:   ``,
		DirModTime: time.Unix(1489730789, 0),
	...
	dir3 := &embedded.EmbeddedDir{
		Filename:   `bootstrap3`,
		DirModTime: time.Unix(1489730777, 0),
		ChildFiles: []*embedded.EmbeddedFile{
