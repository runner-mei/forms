...
func TestRadioButtonRender(t *testing.T) {
	field := RadioField(makeCtx(), "radio", []InputChoice{
...
func TestSelectRender(t *testing.T) {
	field := SelectField(makeCtx(), "select", map[string][]InputChoice{
		"": []InputChoice{
...
func TestNumberRender(t *testing.T) {
	field := NumberField(makeCtx(), "number")
	field.SetStyle(style)
...
func TestInlineCreation(t *testing.T) {
	form := BaseForm(POST, "/action.html").Elements(
		TextField("text_field").SetLabel("Username"),
		FieldSet("psw_fieldset",
			PasswordField("psw1").AddClass("password_class").SetLabel("Password 1"),
			PasswordField("psw2").AddClass("password_class").SetLabel("Password 2"),
		),
		SubmitButton("btn1", "Submit"),
	)
	t.Log("Rendered form:", form.Render())
}
